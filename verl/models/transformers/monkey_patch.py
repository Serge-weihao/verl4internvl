...
def apply_monkey_patch(
    ...
    if model.config.model_type == "qwen2_5_vl":
        ...
        if model.config.llm_config.model_type=="qwen2":
            if hasattr(module, 'Qwen2ForCausalLM'):
                ...
                if ulysses_sp_size > 1:
                    patch_vlm_for_ulysses_input_slicing(module.Qwen2ForCausalLM)
                return
        elif model.config.llm_config.model_type=="xxx":
             pass
        else:
            raise ValueError("internvl  not supported")
