...
def hf_tokenizer(name_or_path, correct_pad_token=True, correct_gemma2=True, **kwargs):
    ...
    if re.match("internvl", config.model_type):
        ...
        if '<xx>' not in tokenizer.chat_template:
            ...
            tokenizer.chat_template="""{% for message in messages %}{{'<xxxxx>:' + ''}}{% if message['content'] is string %}{{ message['content'] }}{% else %}{% for content in message['content'] %}{% if content['type'] == 'image' %}{{ '<image>' }}{% elif content['type'] == 'video' %}{{ '<video>' }}{% elif content['type'] == 'text' %}{{ content['text'] }}{% endif %}{% endfor %}{% endif %}{{''}}{% endfor %}{% if add_generation_prompt %}{{'<xx>:' }}{% endif %}"""
...
def hf_processor(name_or_path, **kwargs):
    ...
    try:
        ...
        print("Processor intitial failed, get error",e)
        processor = None
